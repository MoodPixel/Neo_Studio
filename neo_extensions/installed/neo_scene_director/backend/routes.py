...
from neo_studio_v1.utils.config import TEMPLATES_DIR
from neo_studio_v1.utils.shared_data_paths import studio_data_path
from neo_studio_v1.utils.scene_director import (
    SCENE_DIRECTOR_PACK_ZIP,
    build_scene_director_status,
    default_scene_json,
    workflow_path_by_name,
)
from neo_studio_v1.routes.common import json_error, json_exception
...
# Extension runtime aliases mounted under /api/extensions/runtime/image.scene_director
@router.get('/status')
async def api_scene_director_runtime_status():
    return await api_scene_director_status()

@router.get('/default-scene')
async def api_scene_director_runtime_default_scene(case: str = 'pose_interaction'):
    return await api_scene_director_default_scene(case=case)
