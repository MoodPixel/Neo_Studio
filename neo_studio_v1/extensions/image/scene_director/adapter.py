...
try:
    from neo_studio_v1.utils.shared_data_paths import studio_data_path
except Exception:  # pragma: no cover - extension can run before app imports settle
    studio_data_path = None
...
def _identity_profile_dir() -> Path | None:
    if studio_data_path is None:
        return None
    try:
        return studio_data_path('identity_profiles', legacy_rel='identity_profiles')
    except Exception:
        return None
...
def _compose_scene_director_prompt_context(payload: dict[str, Any], scene: dict[str, Any], contracts: dict[str, Any], count: int) -> dict[str, Any]:
    ...
    if mode == 'global_and_style':
        ...
        # Contracts are already appended separately per region and globally; repeating
        # them inside context can drown out box-specific prompts.
        region_context = effective_global_base
    elif mode == 'global_only':
        region_context = main_prompt
...
def _identity_unit_from_region(region: dict[str, Any], region_index: int) -> dict[str, Any] | None:
    ...
    if profile_id and not _profile_reference_images(profile, region):
    ...
    for index, region in enumerate(active, start=1):
        ...
        if isinstance(identity_unit, dict) and not identity_unit.get('missing_reference_image'):
            ...
            ipadapter[region_id] = {
                ...
                'slot': None,
                'region_mask': True,
                'attn_mask_output_index': 5 + index,
            }
